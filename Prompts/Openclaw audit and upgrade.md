...
The `read_when` field is critical — it tells agents when to load this file without Zeus having to hardcode it. Be specific. Example: `"When your Coder agent receives any frontend build or UI task"` not just `"frontend tasks"`.
...
|---|---|---|
| `vibe-code` | your Coder agent | Any coding, debugging, or build task |
| `frontend-design` | your Coder agent | Any frontend, UI, landing page, or web build task |
| `workflow-creator` | your Coder agent + your Strategy agent | Any automation build, n8n workflow, or AI pipeline task |
| `linkedin-content` | your Content agent | Any LinkedIn post, content creation, or social media task |
| `client-deliverable` | your Strategy agent | Any proposal, SOW, discovery recap, project update, or client doc task |
...
## Voice Profile — your Content agent Specific

`linkedin-content` currently references `NEXTERA.md` for voice guidance. Inside OpenClaw, your Content agent uses `memory/VOICE_PROFILE.md` as the authoritative voice source. In the converted `linkedin-content` skill:
...
- The skill should function as a **format and hook reference** that your Content agent reads during her Step 4 (generate full package) — it should complement her existing workflow, not replace it
- Do not duplicate your Content agent's existing brief → duplicate check → generate → revision loop. The skill provides the LinkedIn-specific format rules and voice calibration only.
...
|---|---|---|---|
| [[VIBE-CODE.md]] | YC vibe coding methodology for AI-assisted sessions | your Coder agent | [date] |
| [[FRONTEND-DESIGN.md]] | Frontend design and build standards | your Coder agent | [date] |
| [[WORKFLOW-CREATOR.md]] | Automation workflow lifecycle — design to documentation | your Coder agent, your Strategy agent | [date] |
| [[LINKEDIN-CONTENT.md]] | LinkedIn content format and voice calibration | your Content agent | [date] |
| [[CLIENT-DELIVERABLE.md]] | Client-facing documents — proposals, SOWs, updates | your Strategy agent | [date] |
```
...
- Add to your Coder agent boot sequence in REGISTRY.md
...
- Wire to both your Coder agent (build phase) and your Strategy agent (planning phase) in REGISTRY.md
...
- Restructure as a your Content agent reference file, not a standalone workflow
- Wire into your Content agent's Context Files in REGISTRY.md

### client-deliverable
- Reformat frontmatter, add `read_when`
- Remove bundled NEXTERA.md copy
- Replace with `[[NEXTERA.md]]` links to specific sections (Section 3 for proof points, Section 5 for pricing, Section 8 for voice)
- Wire into your Strategy agent's Context Files in REGISTRY.md
